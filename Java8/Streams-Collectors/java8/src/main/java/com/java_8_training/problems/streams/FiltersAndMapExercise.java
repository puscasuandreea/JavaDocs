...
public class FiltersAndMapExercise {
    ...
    public static List<Integer> doubleNumbers() {
        ...
        doubleNumbers = input.map(dp -> dp*2).collect(toList());
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        //TODO #6
        // HINT: use map
        collect = personList.stream().map(gn-> gn.getName()).collect(toList());
        return  collect;
    }
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(fla->fla.stream()).collect(toList());
    ...
    public static int product(){
        ...
        int product = 1;
        //TODO #8
        product = input.reduce(1,(pro,x)->pro*x);
    ...
    public static int findTheMinimumNumber(){
        ...
        // HINT: use reduce
        int min =0;
        min = input.reduce(Integer.MAX_VALUE,(pro1,x)->Integer.max(pro1,x));
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        min = personList.stream().map(pers -> pers.getAge()).reduce(Integer.MIN_VALUE,(pers,x)->Integer.min(pers,x));
    ...
    public static double findAgeAverage(List<Person> personList){
        double sum = 0;
        //TODO #13
        personList.stream().map( fa ->fa.getAge()).reduce(0,(a,b)-> a+b);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList
                .stream()
                .filter(pers2 ->pers2.getName().startsWith("J")).map(Person::getNationality)
                .distinct()
                .collect(toList());
    ...
    public static int sumStringLength(){
        ...
        int sum = 0;
        //TODO #16
        sum = strings.stream().mapToInt(String::length).sum();
        return sum;
    }
